<?php 
        ...
        $server = $_POST["kanboard-api-endpoint"];
        $kanboardtoken = $_POST["kanboard-api-token"];
        $trellokey = $_POST["trello-api-key"];
        $trellotoken = $_POST["trello-api-token"];
        $trelloboard = $_POST["trello-board-id"];
        $userId = null;
        ...
        $client->authentication('jsonrpc', $kanboardtoken);
        ...
        $currentTrelloBoardName = $trelloObj->name;

        echo '<p> trello board name: <b>  ' . $currentTrelloBoardName . ' </b></p> ';
        ...
        echo $actual_link . '/kanboard/plugins/TrelloMigrate/Assets/kanboard-import-trello/redirect.php';

        if ( empty($projects) ) {
            // //create the project
            echo 'Creating project.' . PHP_EOL;
            $projectId = $client->createProject($currentTrelloBoardName);
            $counter=0;
            while (empty($projectId)) {
            $projectId = $client->createProject($trelloObj->name.$counter++);
              die("We could not create the project, perhaps it already exists?".PHP_EOL);
        ...
        if ( !empty($projects) ){
            ...
            foreach ($projects as $project) {
                if ($currentTrelloBoardName !== $project['name'] ) {
                  echo "<br/>" . $project['name'] . PHP_EOL;
                  ...
                  $projectId = $client->createProject($trelloObj->name.$counter++);
                    die("We could not create the project, perhaps it already exists?".PHP_EOL);
                     ...
                     echo '<script>window.location.href = "' . $actual_link . '/kanboard/?controller=ConfigController&action=show&plugin=TrelloMigrate&status=0"; </script>';
                     die("We could not create the project, perhaps it already exists?".PHP_EOL);
                }
             }
        }
        ...
        echo '<script>window.location.href = "' . $actual_link . '/kanboard/?controller=ConfigController&action=show&plugin=TrelloMigrate&status=1"; </script>';
        // exit();
        // die;
        ...
        global $userId;
          ...
          $startDate = $card->dateLastActivity !== null ? date('m/d/Y H:i', strtotime($card->dateLastActivity)) : null;
          // $endDate = $card->date !== null ? date('Y-m-d', strtotime($card->date)) : null;
          ...
          if ($startDate !== null) {
        		$params['date_started'] = $startDate;
            	// echo 'date was '. $startDate . PHP_EOL;
        	}
        	if ($colorId !== null) {
